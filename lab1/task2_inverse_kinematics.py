...
def part1_simple(viewer, target_pos):
    ...
    meta_data = MetaData(joint_name, joint_parent,
                         joint_initial_position, 'RootJoint', 'lWrist_end')
...
def part1_hard(viewer, target_pos):
    ...
    joint_name, joint_parent, joint_initial_position = viewer.get_meta_data()
    meta_data = MetaData(joint_name, joint_parent,
                         joint_initial_position, 'lToeJoint_end', 'lWrist_end')
    joint_position = viewer.get_joint_positions()
    joint_orientation = viewer.get_joint_orientations()
...
def part2(viewer, bvh_name):
    ...
    bvh_joint_name, bvh_joint_parent, bvh_offset = part1_calculate_T_pose(
        bvh_name)
    joint_name, _, joint_initial_position = viewer.get_meta_data()
    idx = [joint_name.index(name) for name in bvh_joint_name]
    meta_data = MetaData(bvh_joint_name, bvh_joint_parent,
                         joint_initial_position[idx], 'lShoulder', 'lWrist')
    ...
    class UpdateHandle:
        ...
        def update_func(self, viewer):
            ...
            viewer.show_pose(self.joint_name, joint_position,
                             joint_orientation)
            self.current_frame = (self.current_frame +
                                  1) % self.motion_data.shape[0]
    ...
    class UpdateHandle:
        ...
        def update_func(self, viewer):
            ...
            self.joint_position, self.joint_orientation = bonus_inverse_kinematics(
                meta_data, self.joint_position, self.joint_orientation, left_target_pos, right_target_pos)
            viewer.show_pose(joint_name, self.joint_position,
                             self.joint_orientation)
    handle = UpdateHandle(left_marker, right_marker,
