    ...
    def update(self, task):
        ...
        for i in range(len(self.joint_name)):
            self.viewer.set_joint_position_orientation(self.joint_name[i],
                                                       self.translation[self.cur_frame //
                                                                        speed_inv, i, :],
                                                       self.orientation[self.cur_frame//speed_inv, i, :])
        self.cur_frame = (
            self.cur_frame + 1) % (self.translation.shape[0]*speed_inv)
        return task.cont


def part0_check_joint_name_and_bone_index():
    # 一些不同的设置
    bvh = 'motion_material/idle.bvh'
    idle_bvh = BVHMotion(bvh)
    for i in range(len(idle_bvh.joint_name)):
        print("{}: {}".format(i, idle_bvh.joint_name[i]))


def part1_translation_and_rotation(viewer: SimpleViewer, setting_id):

    # 一些不同的设置
    bvh_list = ['motion_material/walk_forward.bvh',
                'motion_material/idle.bvh', 'motion_material/walk_and_turn_left.bvh']
    ...
    task = ShowBVHUpdate(viewer, new_motion.joint_name,
                         translation, orientation)
    viewer.addTask(task.update)

    # 画些参考点
    viewer.create_arrow(np.array([pos[0], 1e-3, pos[1]]), facing_xz)
    viewer.create_marker(np.array([pos[0], 0, pos[1]]), [0, 0, 0, 1])
    return


def part2_interpolate(viewer, beta):
    ...
    # 计算插值系数
    v1 = (walk_forward.joint_position[-1, 0,
          2] / walk_forward.motion_length) * 100
    v2 = (run_forward.joint_position[-1, 0,
          2] / run_forward.motion_length) * 60
    v = v1 + (v2 - v1) * beta
    print("v1: {} v2: {}".format(v1, v2))
    blend_weight = (v - v1) / (v2 - v1)
    print("blend weight: ", blend_weight)
...
def part3_build_loop(viewer):
    ...
    motion = build_loop_motion(motion, front_blend_weight=0.5)
    motion = motion.translation_and_rotation(
        0, np.array([0, 0]), np.array([0, 1]))
    motion.joint_position[..., 0, 0] = 0.
...
def part4_concatenate(viewer, setting_id):
    ...
    run_forward = BVHMotion('motion_material/run_forward.bvh')
    run_forward = run_forward.translation_and_rotation(
        0, run_forward.joint_position[0, 0][[0, 2]], np.array([0, -1]))
    run_forward.adjust_joint_name(walk_forward.joint_name)
    # idle_ = BVHMotion('motion_material/idle.bvh')
    # idle_ = idle_.translation_and_rotation(0, idle_.joint_position[0, 0][[0, 2]], np.array([0, -1]))

    motion = concatenate_two_motions(walk_forward, run_forward, mix_time, 30)
    print("motion length: ", motion.motion_length)
    translation, orientation = motion.batch_forward_kinematics()
    task = ShowBVHUpdate(viewer, motion.joint_name, translation, orientation)
    viewer.addTask(task.update)
    pass
...
def main():
    ...
    part0_check_joint_name_and_bone_index()
