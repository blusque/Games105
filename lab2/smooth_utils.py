...
def build_loop_motion(bvh_motion, half_life = 0.2, fps = 60, blend_weight = 0.5):
    ...
    for i in range(bvh_motion.motion_length):
        offset1 = decay_spring_implicit_damping_rot(
            fbw * rot_diff, fbw * avel_diff, half_life, i/fps
        )
        offset2 = decay_spring_implicit_damping_rot(
            -bbw * rot_diff, -bbw * avel_diff, half_life, (bvh_motion.motion_length-i-1)/fps
        )
    ...
    for i in range(bvh_motion.motion_length):
        offset1 = decay_spring_implicit_damping_pos(
            fbw * pos_diff, fbw * vel_diff, half_life, i/fps
        )
        offset2 = decay_spring_implicit_damping_pos(
            -bbw * pos_diff, -bbw * vel_diff, half_life, (bvh_motion.motion_length-i-1)/fps
        )
