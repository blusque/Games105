...
def part1_pd_control(viewer, setting=0):
    ...
    motion_list = [r'motion_material\physics_motion\long_walk.bvh',
                   r"motion_material\idle.bvh", r"motion_material\walkF.bvh"]
    ...
    viewer.set_pose(motion.joint_name,
                    joint_translation[0], joint_orientation[0])
...
def part2_root_force(viewer, setting=0):
    ...
    pd_controller = PDController(viewer)
    def idx_map(x): return x//viewer.substep if setting == 1 else 0
    pd_controller.get_pose = lambda x: (
        motion.joint_position[idx_map(x)], motion.joint_rotation[idx_map(x)], setting)
